...
public class FallenFirLogFeature extends Feature<NoneFeatureConfiguration>
{
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> featurePlaceContext)
    {
        ...
        for (int i = 0; i < length; i++)
        {
            this.setBlock(world, pos.relative(direction, i), CBlocks.RANDOM_BLOCK.get().defaultBlockState().setValue(RotatedPillarBlock.AXIS, direction.getAxis()));
            ...
            if (blockAbove.isAir() || blockAbove.getBlock() instanceof BushBlock || blockAbove.getBlock() == Blocks.SNOW)
            {
                if (rand.nextInt(5) == 0)
                {
                    this.setBlock(world, pos.above().relative(direction, i), CBlocks.RANDOM_BLOCK.get().defaultBlockState());
                }
