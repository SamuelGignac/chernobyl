            ...
            for (int l = pos.getZ() - j; l <= pos.getZ() + j; ++l)
            {
                ...
                if (i1 * i1 + j1 * j1 <= j * j)
                {
                    for (int k1 = pos.getY() - 2; k1 <= pos.getY() + 2; ++k1)
                    {
                        ...
                        if (blockstate.getBlock() == CBlocks.RANDOM_BLOCK.get() && this.isAir(worldIn, blockpos.above()))
                        {
                            if (rand.nextInt(2) > 0)
                            {
                                worldIn.setBlock(blockpos, CBlocks.RANDOM_BLOCK.get().defaultBlockState(), 2);
