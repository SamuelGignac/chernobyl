...
public class ModConfig
{
    ...
    public static void setup()
    {
        ...
        ModLoadingContext.get().registerConfig(net.minecraftforge.fml.config.ModConfig.Type.COMMON, GenerationConfig.SPEC, "Chernobyl/generation.toml");
    }

    public static boolean isBiomeEnabled(ResourceKey<Biome> key)
    {
        if (key == null || !key.location().getNamespace().equals(Chernobyl.MODID))
            return false;
    ...
    private static void createConfigDirectoryIfNecessary()
    {
        ...
        {
            Chernobyl.LOGGER.error("Failed to create Chernobyl config directory", e);
        }
    }
    ...
    private static Path getBOPConfigPath()
    {
        ...
        return Paths.get(configPath.toAbsolutePath().toString(), "Chernobyl");
    }
    ...
    static
    {
        defaultBiomeToggles.putAll(CBiomes.getAllBiomes().stream().collect(Collectors.toMap(ModConfig::getBiomeConfigOptionName, key -> true)));
    }
}
